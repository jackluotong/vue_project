<template>
  <div style="background:#ECECEC; padding:20px;">
    ...
    >
      <div class="content">
        <ol>
          <li v-for="(item, index, id) in interview" :key="id" class="liClass">
            {{ item.index }}{{ item.title }} <span>题目：</span> {{ item.name
            }}<br />答案：<span style="color:blue">{{ item.answer }}. </span>
          </li>
        </ol>
      </div>
    </a-card>
    ...
    <div style="paddingTop:10px">
      ...
      <div>
        <p class="test">
          1111111111111111111111111111111111122测试超出部分怎么实现省略号...
        </p>
      </div>
    </div>
  </div>
    ...
    return {
      ...
      interview: [
        ...
        { name: '文档流？', answer: '' },
        {
          name: '数组的去重排序？',
          answer:
            '去重:set,sort,includes,hasOwenProperty,filter,indexOf,递归,map;排序:冒泡，选择，快排，希尔，递归，插入，桶',
        },
        {
          name: '回流和重绘？',
          answer:
            '回流是当几何属性发生改变时产生的；重绘是当样式等外观发生改变时产生的',
        },
        {
          name: 'poistion的属性有哪几种？',
          answer: 'fixed,sticky,relative,absolute,static',
        },
        { name: 'canvas?', answer: '' },
        ...
        { name: '堆栈的差距？', answer: '' },
        {
          title: '01-12面试',
          name: ' vue-router的原理？',
          answer: 'hash：hashchange，history：pushState与replaceState ',
        },
        {
          name: 'vuex中同步和异步是怎么操作的？',
          answer:
            'state getters mutations actions modules,mutations为同步 actions是异步',
        },
        ...
        { name: 'tcp udp的区别？', answer: '' },
        {
          name: '跨域怎么处理？以及jsonp',
          answer:
            'nginx，jsonp，cros，localstorage，sessionstorage，jsonp原理：利用src属性来实现跨域',
        },
        ...
        { name: 'react中的virtual dom 以及diff？', answer: '' },
        {
          name: 'websocket？',
          answer:
            '双向协议，http只能是客户端发起，建立在tcp只上。不受同源策略限制，',
        },
        ...
        { name: 'vuex和全局变量的差别？', answer: '' },
        {
          name: 'js中的编译是什么意思？',
          answer:
            '语法分析js引擎检查你的代码有没有什么低级的语法错误预编译预编译简单理解就是在内存中开辟一些空间，存放一些变量与函数遇到函数时 预编译什么时候发生?遇到函数的时候其实只有在执行函数的时候才会预编译那一部分解释执行执行代码',
        },
        {
          name: '强类型和若类型的语言差异？',
          answer: '强类型语言是需要提前声明数据类型弱类型不需要',
        },
        {
          title: '01-20叮咚买菜面试',
          name: ' jsonp的原理？',
        ...
        },
        {
          name: '图片懒加载的原理？',
          answer:
            'src属性决定是否加载照片=》可视区的判断：getBoundingClientRect()//获取元素大小以及位置',
        },
        {
          name: 'cors中后端需要配置什么？',
          answer:
        ...
        },
        {
          name: '跨域问题？',
          answer:
            'jsonp,CORS,Nginx,window.postMessage,window.name,document.domain,后台配置解决跨域',
        },
        { name: '解构赋值中怎么取最后一个值？', answer: '' },
        {
          title: '01-20MicroFocus面试',
          name: ' button以及input中的button的区别？',
          answer:
            '在表单以外的地方使用<button>标签，表现为正常的按钮，可以用做事件触发器;在表单中使用<button>标签，但未指定type属性值，会像type值为submit的input控件一样完成表单提交动作;',
        },
        { name: '数组中的对象拿到值如何比较？', answer: '' },
        {
          title: '01.26面试',
          name: 'vue的双向绑定以及m怎么到v,v怎么到m',
          answer: '',
        },
        {
          name: '浏览器的缓存？',
          answer:
            '在客户端和服务端通讯的时候，浏览器第一次向服务器发起该请求后拿到请求结果，会根据响应报文中HTTP头的缓存标识，决定是否缓存结果，是则将请求结果和缓存标识存入浏览器缓存中',
        },
        { name: '浏览器的缓存中的etag？', answer: '' },
        {
          name: 'link,import区别？',
          answer: 'link的同时被加载，import的当前页面加载完毕时被加载',
        },
        ...
        },
        {
          name: '成千上万条数据怎么优化？',
          answer:
            '从数据上处理：分页分表，比如前端可以把数据分页展示，后端也分段吐数据从渲染上解决，异步渲染，比如进入页面先不渲染，然后加载好页面再渲染，局部渲染：只渲染目前可见区域的数据，再渲染次屏数据，还有性能瓶颈，可以考虑web worker 做压缩和解码，也可以考虑离屏canvas做预渲染，减少网络耗时：压缩数据，gzip等',
        },
        ...
        {
          name: 'vue2为什么不能监听数组？是如何解决的？',
          answer:
            '出于性能的考虑，数据一般会被频繁的改动，每次的改动都需要遍历整个数组，给数组属性重新observe，这样会极大的消耗性能，因此在Vue2中hack了Array上的一些方法',
        },
        {
          name: 'vue3是如何监听数组的？',
          answer:
            'Proxy来进行对象、数组的代理；只能对属性拦截，对象或数组监测时必须遍历，另一个是对对象本身的劫持',
        },
        ...
        { name: '跨域是发生在浏览器还是服务器？', answer: '浏览器' },
        {
          name: 'spa页面的优缺点？',
          answer:
            '优点:一次加载完成，不会因为用户的操作而进行页面的重新加载或跳转，前后端分离 减少开发效率。架构清晰，减轻服务器的压力；缺点：对SEO并不友好，可能出现首屏加载时间过长的问题，页面复杂度提高。复杂逻辑程度成倍。',
        },
        {
          name: 'nextick原理？',
          answer: '响应式并不是数据发生变化之后 DOM 立即变化，',
        },
        { name: 'h5新增的操作dom的api？', answer: '' },
        {
          name: 'computed和watch的区别？',
          answer:
            'computed：支持缓存，只有依赖数据发生改变，才会重新进行计算，不支持异步，一个属性是由其他属性计算而来的，这个属性依赖其他属性，是一个多对一或者一对一，一般用computed，如果computed属性属性值是函数，那么默认会走get方法；函数的返回值就是属性的属性值；在computed中的，属性都有一个get和一个set方法，当数据变化时，调用set方法。watch：不支持缓存，数据变，直接会触发相应的操作，watch支持异步，监听的函数接收两个参数，第一个参数是最新的值；第二个参数是输入之前的值，',
        },
        ...
        {
          title: '01.29哈罗出行面试',
          name: 'css中的position？以及其中的relative与absolute的区别',
          answer: '',
        },
